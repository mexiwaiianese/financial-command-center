...
function App(){
  ...
  return <main>
    ...
    <section className="panel"><h2>ADU Progress</h2><div className="adu-grid"><KpiCard label="Budget" value={fmt(adu.budget)}/><KpiCard label="Spent" value={fmt(adu.spent)}/></div><div className="progress-wrap"><div className="progress-label">{adu.progress||0}%</div><div className="progress"><div style={{width:`${adu.progress||0}%`}}/></div></div></section>
    <section className="panel"><h2>Recent Transactions</h2><table><thead><tr><th>Date</th><th>Description</th><th>Category</th><th>Amount</th></tr></thead><tbody>{transactions.slice(0,20).map(t=><tr key={t.id}><td>{t.date}</td><td>{t.description}</td><td>{t.category}</td><td>{fmt(t.amount)}</td></tr>)}</tbody></table></section>
  </main>;
}
